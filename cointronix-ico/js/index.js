...
var props = {
	...
	globeRadius: 400, // Radius of the globe (used for many calculations)
...
function getData() {
	...
	request.open('GET', 'js/globe-points.json', true);

	request.onload = function() {
		if (request.status >= 200 && request.status < 800) {
...
function addGlobe() {
	...
	canvasGradient.addColorStop(0, '#010877');
	canvasGradient.addColorStop(1, '#b121fb');
	canvasGradient.addColorStop(1, '#002fd8');
...
function introAnimate() {
	...
	if (animations.dots.current >= (animations.dots.total * 0.15) && animations.globe.current <= animations.globe.total) {
